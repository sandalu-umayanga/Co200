...
export default function P_search() {
    ...
    const [currentReportIndex, setCurrentReportIndex] = useState(0); // To track current report

    const handleSearch = async (event) => {
        setSearchTerm(event.target.value);
        try {
            const response = await api.get(`/api/patient/search/?query=${event.target.value}&colum=${searchBy}`);
